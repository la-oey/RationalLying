...
#write_csv(bs.final, "bsfinal.csv")
...
```{r}
morality <- read_csv("sims_varyMorality_expt4_alph0.25.csv")
glimpse(morality)
moralityResponseToIdiot <- read_csv("sims_varyMorality_liarResponseToIdiot_expt4_alph0.25.csv")
...
write_csv(df.unif, "sims_varyMorality_expt4_alph0.75.csv")

# Recursive Model w/ Morality Penalty

lie.recurse.moral <- df.unif %>%
  filter(role=="Liar", mor==19) %>%
  mutate(p = paste("p =", p)) %>%
  ggplot(aes(x=ks, y=val, colour=p)) +
  geom_abline(size=3, intercept = 0, slope = 1, colour="darkgray", linetype=2) +
  geom_line(size=2.5, alpha=0.8) +
  ggtitle("Rational + Morality") +
  # scale_x_continuous("True Marbles Drawn", breaks=c(0,2,4,6,8,10)) +
  # scale_y_continuous("Reported Marbles Drawn", limits=c(0,10), breaks=c(0,2,4,6,8,10)) +
...
detect.recurse.moral <- df.unif %>%
  filter(role=="Detector", mor==11) %>%
  mutate(p = paste("p =", p)) %>%
  ggplot(aes(x=ks, y=val, colour=p)) +
  geom_line(size=2.5, alpha=0.8) +
  ggtitle("") +
  # scale_x_continuous("Reported Marbles Drawn", breaks=c(0,2,4,6,8,10)) +
  # scale_y_continuous("Proportion of Calling BS", limits=c(0,1)) +
  scale_x_continuous("", breaks=c(0,2,4,6,8,10)) +
...
lie.respondToHumanData <- bsPropsDF %>%
  ...
  geom_abline(intercept = 0, slope = 1, colour="darkgray", linetype=2, size=3) +
  geom_line(size=2.5, alpha=0.8) +
  ggtitle("Response to Human Data") +
  # scale_x_continuous("True Marbles Drawn", breaks=c(0,2,4,6,8,10)) +
  # scale_y_continuous("Expected Reported", limits=c(0,10), breaks=c(0,2,4,6,8,10)) +
  scale_x_continuous("", breaks=c(0,2,4,6,8,10)) +
  scale_y_continuous("", limits=c(0,10), breaks=c(0,2,4,6,8,10)) +
...
for(i in c(0.2, 0.5, 0.8)){
  for(j in -5:25){
    moral = j
    expLie <- exp.ksay(i, rep(avgPropBS, 11))
    simLieRespondToIdiot <- bind_rows(simLieRespondToIdiot, data.frame(role="Liar", p=i, mor=j, ks=0:10, n=numSims, val=expLie))
  }
}
write_csv(simLieRespondToIdiot, "sims_varyMorality_liarResponseToIdiot_expt4_alph0.75.csv")


lie.respondToIdiot <- data.frame(p=c(rep(0.2,11), rep(0.5,11), rep(0.8,11)),
           k=rep(0:10,3),
           ksay=c(exp.ksay(0.2, rep(avgPropBS, 11)),
                  exp.ksay(0.5, rep(avgPropBS, 11)),
                  exp.ksay(0.8, rep(avgPropBS, 11)))) %>%
           # ksay=c(exp.ksay(0.2, pbinom(0:10, 10, 0.2)),
           #        exp.ksay(0.5, pbinom(0:10, 10, 0.5)),
           #        exp.ksay(0.8, pbinom(0:10, 10, 0.8)))) %>%
  mutate(probabilityRed.txt = paste("p =", p)) %>%
  ggplot(aes(x=k, y=ksay, colour=probabilityRed.txt)) +
  geom_abline(intercept = 0, slope = 1, colour="darkgray", linetype=2, size=3) +
  geom_line(size=2.5, alpha=0.8) +
  ggtitle("Response to Uniform") +
  # scale_x_continuous("True Marbles Drawn", breaks=c(0,2,4,6,8,10)) +
  # scale_y_continuous("Expected Reported", limits=c(0,10), breaks=c(0,2,4,6,8,10)) +
  scale_x_continuous("", breaks=c(0,2,4,6,8,10)) +
  scale_y_continuous("", limits=c(0,10), breaks=c(0,2,4,6,8,10)) +
...
detect.idiot <- data.frame(p=c(rep(0.2,11), rep(0.5,11), rep(0.8,11)),
  ...
  ggtitle("") +
  scale_x_continuous("Reported Marbles Drawn", breaks=c(0,2,4,6,8,10)) +
  scale_y_continuous("Proportion of BS Calling", limits=c(0,1)) +
  # scale_x_continuous("", breaks=c(0,2,4,6,8,10)) +
  # scale_y_continuous("", limits=c(0,1)) +
  scale_colour_manual(values=condColors.diff)+
  theme_minimal() +
...
lie.results <- bs.final %>%
  ...
  # scale_x_continuous("True Marbles Drawn", limits=c(-0.2,10.2), breaks=c(0,2,4,6,8,10)) +
  # scale_y_continuous("Reported Marbles Drawn", limits=c(0,10), breaks=c(0,2,4,6,8,10)) +
  scale_x_continuous("", limits=c(-0.2,10.2), breaks=c(0,2,4,6,8,10)) +
  scale_y_continuous("", limits=c(0,10), breaks=c(0,2,4,6,8,10)) +
  scale_colour_manual(values = condColors.diff) +
  scale_fill_manual(values = condColors.diff) +
  guides(fill=guide_legend(title="", override.aes=list(size=15)), colour="none") +
  theme_minimal() +
  theme(legend.text = element_text(size=24),
        ...
        axis.title = element_text(size=28),
        axis.text = element_text(size=24))
lie.results + theme(legend.position="none")
...
# Combine Graphs
```{r}
legend <- get_legend(lie.results)
# allplots <- grid.arrange(lie.idiot, detect.idiot, 
#                          lie.respondToIdiot, detect.respondToIdiot, 
#                          lie.respondToHumanData, detect.respondToHumanData, 
#                          lie.recurse.nomoral, detect.recurse.nomoral, 
#                          lie.recurse.moral, detect.recurse.moral, 
#                          lie.results + theme(legend.position="none"), detect.results, 
#                          nrow=6)
allplots <- grid.arrange(lie.idiot, 
                         lie.respondToIdiot, 
                         lie.respondToHumanData, 
                         lie.recurse.nomoral, 
                         lie.recurse.moral, 
                         ...
                         detect.results, 
                         nrow=2)
withLegend <- plot_grid(allplots, legend, rel_widths=c(9, 1))
ggsave("img/allPredictions.png", 
       withLegend, 
       height=12, width=30)
