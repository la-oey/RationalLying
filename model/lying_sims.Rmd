...
ideal <- grid.arrange(lie.recurse, detect.recurse, nrow=2)
...
zeroth_0.5_uncorrected <- pbinom(0:numMarbles, numMarbles, 0.5)

oracle <- data.frame(ksay=0:10,
                     p.D=zeroth_0.5,
                     p.D2=zeroth_0.5_uncorrected,
                     exp.L=first_0.5)
oracle_lie <- ggplot(oracle, aes(x=ksay, y=exp.L)) +
  ...
  scale_y_continuous("", limits=c(0,10)) +
  theme_minimal()
oracle_detect <- ggplot(oracle, aes(x=ksay, y=p.D2)) +
  geom_line() +
...
# dumbLiar <- data.frame(real=0:10, reported = rep(10,11)) %>%
#   ggplot(aes(x=real, y=reported)) +
#   geom_line(alpha=0) +
#   geom_abline(intercept = 0, slope = 1, colour="red", linetype=2) +
#   scale_x_continuous("") +
...
#   theme_minimal()
# 
# dumbDetect.hypoth <- data.frame(reported=0:10, p.bs.ksay=rep(0.9,11)) %>%
#   ggplot(aes(x=reported, y=p.bs.ksay)) +
#   geom_line(alpha=0) +
#   scale_x_continuous("") +
#   scale_y_continuous(bquote("Expected" *~P[D]* "(BS | k*)"), limits=c(0,1)) +
...
fig.prob <- plot_grid(lie.recurse.p, detect.recurse.p, align = "h", ncol = 2, rel_widths = c(9/20, 11/20))
ggsave("img/prob.png", fig.prob, height=5, width=10)
...
#zeroth.detector <- mapply(hypothesis.test, 0:10, 0.5)
zeroth.liar.p <- p_t.ksay.r(0.5, zeroth.detector)
#zeroth.liar <- rep(10,11)
...
```{r}
data.frame(real=0:10, reported=rep(10,11)) %>%
  ggplot(aes(x=real, y=reported)) +
  geom_line(lwd=1) +
  geom_abline(intercept = 0, slope = 1, colour="red", linetype=2, lwd=1) +
  scale_x_continuous("") +
  scale_y_continuous("Expected Reported (k*)", limits=c(0,10)) +
  theme_minimal()
ggsave("img/highestLie.png")

data.frame(real=0:10, reported=0:10) %>%
  ggplot(aes(x=real, y=reported)) +
  geom_line(lwd=1) +
  geom_abline(intercept = 0, slope = 1, colour="red", linetype=2, lwd=1, alpha=0.8) +
  scale_x_continuous("") +
  scale_y_continuous("Expected Reported (k*)", limits=c(0,10)) +
  theme_minimal()
ggsave("img/alwaysTruth.png")

data.frame(real=0:10, reported=rep(0,11)) %>%
  ggplot(aes(x=real, y=reported)) +
